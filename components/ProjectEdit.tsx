...
import type { Project, ProjectType, ProjectStage, ProjectEntryRoute, UpdateProjectPayload } from '../api';

interface ProjectEditProps {
  project: Project;
  onNavigate: (page: string) => void;
  onUpdateProject: (payload: UpdateProjectPayload) => Promise<void>;
...
const ProjectEdit: React.FC<ProjectEditProps> = ({
  ...
  const [stage, setStage] = useState<ProjectStage | null>(project.stage || null);
  const [brief, setBrief] = useState(project.brief || '');
  const [entryRoute, setEntryRoute] = useState<ProjectEntryRoute | null>(project.entryRoute || null);
    ...
    <div className="min-h-screen bg-white pt-20">
      ...
      <div className="max-w-screen-md mx-auto px-6 py-8">
        ...
        <div className="mt-16 pt-8 border-t border-gray-200">
          <h2 className="font-display text-lg font-bold uppercase tracking-tight text-gray-900 mb-2">
            Danger Zone
          </h2>
          <p className="font-mono text-xs text-gray-500 mb-4">
            Deleting a project will archive it and remove it from your dashboard. This can be undone by contacting support.
          </p>
          ...
          {!showDeleteConfirm ? (
            <button
              ...
              disabled={isProcessing}
              className={`inline-flex items-center gap-2 px-4 py-2 border border-red-300 text-red-600 font-mono text-[10px] tracking-widest uppercase hover:bg-red-50 transition-colors ${
                isProcessing ? 'opacity-50 cursor-not-allowed' : ''
              }`}
            >
            ...
            <div className="bg-red-50 border border-red-200 p-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={isDeleting}
                  className="px-4 py-2 bg-red-600 text-white font-mono text-[10px] tracking-widest uppercase hover:bg-red-700 transition-colors"
                >
                  {isDeleting ? (
                ...
                <button
                  ...
                  disabled={isDeleting}
                  className="px-4 py-2 border border-gray-300 text-gray-600 font-mono text-[10px] tracking-widest uppercase hover:bg-gray-100 transition-colors"
                >
                  Cancel
                </button>
              </div>
